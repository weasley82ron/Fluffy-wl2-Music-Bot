...
class owner(commands.Cog):
    ...
    async def bl_add(self, ctx, user: discord.User):
        ...
        if user.id in [
            ...
            return await ctx.send("You Cannot Bl That User")
        if blacklisted:
            embed = discord.Embed(
                description=f"**{user.name}** Is already in blacklist.",
                color=self.color,
            )
            await ctx.reply(embed=embed, mention_author=False)
        else:
            ...
            self.con.commit()
            embed = discord.Embed(
                description=f"I will now ignore messages from **{user.name}**",
                color=self.color,
            )
            await ctx.reply(embed=embed, mention_author=False)
            async with aiohttp.ClientSession() as session:
    ...
    async def bl_remove(self, ctx, user: discord.User):
        ...
        if blacklisted:
            ...
            self.con.commit()
            embed = discord.Embed(
                description=f"I will no longer ignore messages from **{user.name}**",
                color=self.color,
            )
            await ctx.reply(embed=embed, mention_author=False)
            async with aiohttp.ClientSession() as session:
    ...
    async def setprefix(self, ctx, prefix=None):
        ...
        if prefix is None:
            embed = discord.Embed(
                description=f"Please provide a prefix to update.", color=self.color
            )
            await ctx.reply(embed=embed, mention_author=False)
            return
        if len(prefix) > 2:
            embed = discord.Embed(
                description="Prefix cannot be greater than 2 characters.",
                color=self.color,
            )
            await ctx.reply(embed=embed, mention_author=False)
            return
        cursor.execute(
            f"UPDATE Prefix SET prefix = ? WHERE guild_id = ?", (prefix, ctx.guild.id)
        )
        embed = discord.Embed(
            description=f"Successfully set the prefix to `{prefix}`", color=self.color
        )
        await ctx.reply(embed=embed, mention_author=False)
        self.con.commit()
