...
class events(commands.Cog):
    ...
    async def on_guild_join(self, guild: discord.Guild):
        if guild.id in [1207500001651990549]:
            ...
            async with aiohttp.ClientSession() as session:
                webhook = discord.Webhook.from_url(
                    url=Fluffy.guild_join, session=session
                )
                embed = discord.Embed(
    ...
    async def on_message(self, message, ctx):
        ...
        if message.content == self.client.user.mention:
            ...
            support_button = discord.ui.Button(
                label="Support", url="https://discord.gg/EBjchQXFjW"
            )

            Invite_button = discord.ui.Button(
                label="Invite",
                url="https://discord.com/api/oauth2/authorize?client_id=1206537119472816249&permissions=8&scope=bot",
            )
