...
class info(commands.Cog):
    ...
    async def uptime(self, ctx):
        ...
        uptime_str = f"<t:{uptime_timestamp}:R>"
        embed = discord.Embed(
            description=f"I am online from {uptime_str}", colour=self.color
        )
        await ctx.reply(embed=embed, mention_author=False)

    @commands.command(
        aliases=["latency"], help="See The Realtime Latency Of The Bot", usage="ping"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def ping(self, ctx):
        ...
        embed.set_author(
            name=f"{latency}ms Pong! ", icon_url=ctx.author.display_avatar.url
        )
