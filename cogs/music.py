...
class music(commands.Cog):
    ...
    @commands.command(
        aliases=["begin"], help="Resume the Current Music!", usage="resume"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
        ...
        if not ctx.voice_client:
            ...
            if vc.is_paused():
                ...
                embed4 = discord.Embed(
                    description="Resuming the player now <3", colour=self.color
                )
    ...
    async def stop(self, ctx):
        if not ctx.voice_client:
            ...
            if ctx.author.voice.channel != vc.channel:
                embed3 = discord.Embed(
                    description="You are in not the same voice channel.",
                    colour=self.color,
                )
                return await ctx.reply(embed=embed3, mention_author=False)
            if self.queuee:
            ...
            await vc.disconnect()
            embed4 = discord.Embed(
                description="Stopped and Disconnected :/ ", colour=self.color
            )
    ...
    async def queue(self, ctx):
        ...
        if not vc.is_playing():
            embed3 = discord.Embed(
                description="I am not playing any song.", colour=self.color
            )
            return await ctx.reply(embed=embed3, mention_author=False)
        if ctx.author.voice.channel != vc.channel:
            embed4 = discord.Embed(
                description="You are in not the same voice channel.", colour=self.color
            )
            return await ctx.reply(embed=embed4, mention_author=False)
    ...
    async def volume(self, ctx: commands.Context, volume: int):
        ...
        if not 0 <= volume <= 200:
            embed5 = discord.Embed(
                description="Please provide a volume to set in between 0 to 200.",
                colour=self.color,
            )
            await ctx.reply(embed=embed5, mention_author=False)
            return
        ...
        embed6 = discord.Embed(
            description=f"Volume set to {volume}%", colour=self.color
        )
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        vc.queue.clear()
        embed5 = discord.Embed(
            description="Successfully Cleared The Queue.", colour=self.color
        )
        await ctx.reply(embed=embed5, mention_author=False)

    @commands.command(
        aliases=["dvol"], help="Shows The Default Volume", usage="defaultvolume"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def defaultvolume(self, ctx: commands.Context):
        ...
        embed5 = discord.Embed(
            description="Default volume set to 100%", colour=self.color
        )
        await ctx.reply(embed=embed5, mention_author=False)
    ...
    async def join(self, ctx: commands.Context):
        ...
        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                embed2 = discord.Embed(
                    description=f"I am already in another voice channel",
                    colour=self.color,
                )
                return await ctx.reply(embed=embed2, mention_author=False)
            else:
                embed3 = discord.Embed(
                    description=f"Sucessfully Joined voice channel", colour=self.color
                )
            ...
            embed4 = discord.Embed(
                description=f"Successfully Joined your voice channel", colour=self.color
            )
            return await ctx.reply(embed=embed4, mention_author=False)

    @commands.command(
        aliases=["shift"], help="Moves The Player To Your Channel", usage="move"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
        ...
        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                if ctx.voice_client.is_playing():
                    embed2 = discord.Embed(
                        description="I am currently playing in another voice channel.",
                        colour=self.color,
                    )
                    return await ctx.reply(embed=embed2, mention_author=False)
                else:
                    ...
                    embed3 = discord.Embed(
                        description=f"Successfully moved to **{ctx.author.voice.channel.name}**",
                        colour=self.color,
                    )
                    await ctx.reply(embed=embed3, mention_author=False)
            else:
                embed4 = discord.Embed(
                    description=f"I am already in your voice channel: {ctx.voice_client.channel.name}",
                    colour=self.color,
                )
                await ctx.reply(embed=embed4, mention_author=False)
        else:
            embed5 = discord.Embed(
                description="I am not in a voice channel.", colour=self.color
            )
            await ctx.reply(embed=embed5, mention_author=False)
    ...
    async def leave(self, ctx: commands.Context):
        ...
        await ctx.voice_client.disconnect()
        embed4 = discord.Embed(
            description="Sucessfully Left voice channel.", colour=self.color
        )
        await ctx.reply(embed=embed4, mention_author=False)
    ...
    async def forward(self, ctx):
        ...
        embed6 = discord.Embed(
            description="Skipped the track by 10 seconds.", colour=self.color
        )
        await ctx.reply(embed=embed6, mention_author=False)
    ...
    async def seek(self, ctx, *, time_str):
        ...
        if not match:
            embed6 = discord.Embed(
                description="Invalid time format. Please use either `mm:ss` or `ss`.",
                colour=self.color,
            )
        ...
        if match.group(1):
            ...
            time_seconds = sum(
                c * 60**i for i, c in enumerate(reversed(time_components))
            )
            await vc.seek(time_seconds * 1000)
            embed7 = discord.Embed(
                description=f"Successfully sought to {time_str}.", colour=self.color
            )
            await ctx.reply(embed=embed7, mention_author=False)

    @commands.command(
        aliases=["detach"], help="Remove a Track From The Queue", usage="remove <index>"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def remove(self, ctx, index: int):
        ...
        if not vc.queue or index > len(vc.queue) or index < 1:
            embed6 = discord.Embed(
                description=f"Invalid index. Must be between 1 and {len(vc.queue)}",
                color=self.color,
            )
            return await ctx.reply(embed=embed6, mention_author=False)
        ...
        embed7 = discord.Embed(
            description=f"Successfully removed `[{removed.title}]({Fluffy.support_link})` from Queue.",
            color=self.color,
        )
        await ctx.reply(embed=embed7, mention_author=False)
