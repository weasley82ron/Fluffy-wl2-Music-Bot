...
class Fluffy(commands.AutoShardedBot):
  def __init__(self):
    super().__init__(command_prefix=get_prefix,intents=discord.Intents.all(),case_insensitive=True,strip_after_prefix=True)    
client = Fluffy()
shard_guild_counts = {}
...
async def on_connect():
  await client.change_presence(status=discord.Status.dnd,activity=discord.Activity(type=discord.ActivityType.listening,name="&help"))
...
async def on_ready():
  ...
  client.owner_ids = [1177262245034606647, 897529993914105886]
