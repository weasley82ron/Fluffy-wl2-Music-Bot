...
""",
    ...
    @badge.command(
        aliases=["add"], help="Gives The Badge To A User", usage="give <no.> <user>"
    )
    async def give(self, ctx, badge_number: int = None, user: discord.User = None):
        if not badge_number:
            embed = discord.Embed(
                title="Error",
                color=Fluffy.color,
                description="Please provide a badge number.",
            )

            embed.set_thumbnail(
                url=user.display_avatar.url or ctx.author.display_avatar.url
            )
            embed.set_footer(text="By Fluffy Services", icon_url=Fluffy.icon)
            await ctx.send(embed=embed)
            return
        ...
        if ctx.author.id in badge_givers:
            ...
            if badge_name == "Unknown":
                embed = discord.Embed(
                    title="Error",
                    color=Fluffy.color,
                    description="Not a valid badge! Use `badge list` command.",
                )
            ...
            if badge_number not in user_badges:
                ...
                embed = discord.Embed(
                    title="Badge Given",
                    color=Fluffy.color,
                    description=f"Badge `{badge_name}` given to {user.display_name}",
                )
                ...
                embed = discord.Embed(
                    title="Error",
                    color=Fluffy.color,
                    description=f"{user.display_name} already has badge `{badge_name}`",
                )
    ...
    @badge.command(
        aliases=["rem"], help="Removes The Badge To A User", usage="remove <no.> <user>"
    )
        ...
        if ctx.author.id in badge_givers:
            ...
            if badge_number in user_badges:
                ...
                embed = discord.Embed(
                    title="Badge Removed",
                    color=Fluffy.color,
                    description=f"Badge `{badge_name}` removed from {user.display_name}",
                )
                ...
                embed = discord.Embed(
                    title="Error",
                    color=Fluffy.color,
                    description=f"{user.display_name} doesn't have badge `{badge_name}`",
                )
    ...
    @badge.command(
        aliases=["all"], help="Gives All Badges To A User", usage="giveall <user>"
    )
    async def giveall(self, ctx, user: discord.User = None):
        ...
        if ctx.author.id in badge_givers:
            ...
            await self.save_badges(badges)
            embed = discord.Embed(
                title="All Badges Given",
                color=Fluffy.color,
                description=f"All badges given to {user.display_name}",
            )
    ...
    @badge.command(
        aliases=["rall"],
        help="Removes All Badges From A User",
        usage="removeall <user>",
    )
    async def removeall(self, ctx, user: discord.User = None):
        ...
        if ctx.author.id in badge_givers:
            ...
            if user_id in badges:
                ...
                await self.save_badges(badges)
                embed = discord.Embed(
                    title="All Badges Removed",
                    color=Fluffy.color,
                    description=f"All badges removed from {user.display_name}",
                )
                ...
                embed = discord.Embed(
                    title="Error",
                    color=Fluffy.color,
                    description=f"{user.display_name} doesn't have any badges",
                )
    ...
    @commands.command(
        aliases=["pr"], help="Shows The Current Profile Of User", usage="pr <user>"
    )
    async def profile(self, ctx, user: discord.User = None):
        ...
        if user_badges:
            ...
            embed = discord.Embed(
                title=f"{user.display_name}'s Badges", color=Fluffy.color
            )
            description = ""
            for badge_name in badge_names:
                emoji_name = emojis.get(
                    badge_name, ":white_check_mark:"
                )  # Get emoji from premojis
